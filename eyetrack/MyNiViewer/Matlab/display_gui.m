...
[handles.rgb_image handles.depth_image folder] = display_images(handles.number_index, handles.burst_index);
if(length(handles.rgb_image)>0)
    set(handles.rgb,'NextPlot','add');
    axes(handles.rgb);
    imshow(handles.rgb_image);
%     imwrite(handles.rgb_image, sprintf('%s/Image_%d_%d.png',folder, handles.number_index, handles.burst_index), 'png');
end
if(length(handles.depth_image)>0)
    set(handles.depth,'NextPlot','add');
    axes(handles.depth);
